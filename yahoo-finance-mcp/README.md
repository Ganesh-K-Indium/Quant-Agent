...
This tool performs smart financial analysis beyond just fetching numbers:

*   **Financial Ratios (Health Check)**: It doesn't just give you a balance sheet; it calculates if a company is healthy. It looks at **Liquidity** (can they pay bills?), **Solvency** (how much debt do they have?), and **Profitability** (are they actually making money?).
*   **5-Year Projections (Future View)**: It analyzes the last 5 years of historical data to calculate the **CAGR** (average annual growth). It then uses this trend to project where the company's revenue might be in the next 5 years.
*   **News Sentiment (Vibe Check)**: It reads recent news headlines and uses AI to decide if the news is positive or negative. Based on this, it predicts if the stock is likely to go **UP**, **DOWN**, or stay **STABLE**.

## How it works (The Big Picture)

1.  **Request**: You ask the main system a question (e.g., "Analyze NVIDIA for me").
2.  **Coordination**: The **Stock Agent** realizes it needs financial data and news.
3.  **Data Fetching**: This **Yahoo Finance MCP Tool** is called to do the heavy lifting—it calculates the ratios, projects the growth, and checks the news vibe.
4.  **Final Analysis**: The Agent combines this data with technical charts and gives you a clear, human-friendly answer.
